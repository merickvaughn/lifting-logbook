...
- **Node:** 20.11.1 (managed by nvm for Windows; `.nvmrc` is set — run `nvm use $(cat .nvmrc)` at session start if not already active).
  ...
  - **Windows disk-pressure native crashes under concurrent turbo runs (local only, no code fix):** distinct from the OOM entries above — this one is triggered by severe local disk pressure (`C:` at ~98% full), not by a specific suite's per-worker footprint, and it hits workspaces that have nothing to do with whatever change is being verified. Observed signatures: `tools/eslint-rules#test`'s `node --test` files exiting `3221226505` (`0xC0000409`, Windows `STATUS_STACK_BUFFER_OVERRUN`); a raw V8 fatal crash (`node::OnFatalError`) surfacing directly out of an eslint invocation (`packages/core#lint`); and the existing #807 "Jest worker ran out of memory" signature appearing even under `turbo run test --concurrency=1` (fully serial), not just full-parallel `npm test`. **Diagnosis:** every one of these passes cleanly when run standalone in its own workspace (`npm run test`/`npm run lint` invoked directly, or the single failing Jest suite with `--runInBand`) — a clean standalone pass alongside a turbo-context native crash is the signature of host resource pressure, not a code defect, and no amount of per-suite `workerIdleMemoryLimit` tuning fixes it, because the failing workspace isn't the one under memory pressure. Check `df -h /c` first (the global `CLAUDE.md`'s Disk-Full (ENOSPC) guidance is scoped to *install* truncation; this is the *test-run* analogue of the same root cause). **Fix:** free disk space, then either re-run standalone per failing workspace to confirm, or re-run the full pipeline with `npx turbo run lint typecheck test --concurrency=1` — serializing tasks reduces peak concurrent memory/paging load and was sufficient to get a clean 24/24 run in the reference incident even without freeing additional space beyond a `npm cache clean --force`. Investigation: [#974](https://github.com/merickvaughn/lifting-logbook/issues/974).
...
docs/domain-model.md — Training domain model as built: the lift taxonomy, the
                       Program → Cycle → Week → Workout → Lift slot → Set ladder, and which
                       levels are persisted rows vs. recomputed per request
...
touching the class map and the copy records (compile-enforced), `KIND_RANK` in
`packages/core/src/timer/anchor.ts` (compile-enforced for presence; its *value* — where the kind
sits in a set's emission order, which re-anchoring depends on — is checked by `anchor.test.ts`), a
`.<kind> { stroke: … }` rule in `TimerDial.module.css`, `DIAL_PHASES` in the guard, and a new token in
**each** theme block; the tests' per-kind tables are derived from `TIMER_PHASE_KINDS` and pick it up
automatically. See [#960](https://github.com/merickvaughn/lifting-logbook/issues/960),
