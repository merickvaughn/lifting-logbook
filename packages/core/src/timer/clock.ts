 ...
 * than wrapping, and to `0` for a zero-length phase (which would otherwise
 * divide by zero).
 */
 ...
 *
 * Accepts `M:SS` and bare seconds, since both are natural to type into a box
 * showing `4:00`. Returns `null` for anything unparseable so the caller can keep
 * the previous value rather than silently resetting the field to zero.
 */
