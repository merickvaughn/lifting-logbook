 ...
 *
 * **Several vocabularies reach this lookup, not one.** A built-in 5/3/1 or RPT
 * template's spec `lift` values are `DEFAULT_SLOT_MAP` slot names ("Squat",
 * "Cable Curls"), which cover only a few of the catalog's display names. A *custom*
 * program's `lift` values come from `ProgramEditor`'s exercise picker, which is built as
 ...
 * `liftOverrides` circulates as an id. Per-entry **aliases** cover the names the
 * Leangains and RPT presets use that are neither ("Weighted Pull-ups", "Calf Raises") —
 * see `CatalogLift.aliases` for why those stay out of `DEFAULT_SLOT_MAP`.
 *
 ...
 * opinion" — a custom lift, or a name the app has never seen.
 */
