...
| [`observability/tempo-setup.test.mjs`](observability/tempo-setup.test.mjs) | `node --test` coverage for `tempo-setup.sh`: one managed block on a first run, replacement rather than duplication on re-run, unrelated `~/.bashrc` content (including the sibling `mimir-query-env` block) surviving the rewrite, the written-if-absent backup anchor, `%q` round-tripping of values with shell metacharacters, a refusal saving nothing, and the sourced-export path. Runs the real script against a sandboxed `HOME`, asserting the sandbox took effect before anything else. Runs in CI (`ci.yml`) ([#949](https://github.com/merickvaughn/lifting-logbook/issues/949)). |
