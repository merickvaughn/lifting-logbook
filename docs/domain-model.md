...
**A `replace` swaps the movement, not the slot.** The replacement keeps the slot's
position and its logged sets (records under the original name are grouped under the
replacement). It also keeps the slot's prescription: sets, reps, AMRAP, warm-up and
decrement percentages, increment and activation. Only the weights change, because they
are priced from the replacement's own training max. The workout response names the
slot on the replacement's `replaces` field, following a chain of swaps back to the
spec's lift, so clients resolve the prescription without the override table (#1014).
...
| # | Finding | Location |
|---|---|---|
| **D1** | **Logged `amrap` is recovered by string-sniffing free text**: `amrap: r.notes.toUpperCase().includes('AMRAP')`. Any note mentioning "amrap" flips the flag. `amrap` is a real column on `custom_program_spec` but has none for a logged set. | `apps/api/src/programs/mappers.ts:375` |
...
| **D10** | **`WorkoutResponse.bodyWeightEntry` is declared and never populated** — one source reference repo-wide, the declaration itself. | `packages/types/src/api.ts:123` |
